...
import { X, Save, Bot, Terminal, Zap, Globe, Cloud, Layout, Cpu, Image, Network, Keyboard, Power, MessageSquare } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'llm' | 'cloud' | 'general'>('llm');
      ...
      <div className="w-full max-w-3xl h-[550px] bg-white/80 backdrop-blur-3xl rounded-2xl shadow-macos-window border border-white/40 flex overflow-hidden animate-in zoom-in-95 duration-200 z-10 text-macos-text">
        ...
        <div className="w-56 bg-macos-sidebar border-r border-white/20 flex flex-col p-4 pt-8">
           ...
           <div className="mt-auto pt-4 border-t border-black/5">
             <button 
               onClick={onClose} 
               className="w-full py-2 text-sm font-medium text-macos-muted hover:text-macos-text hover:bg-black/5 rounded-lg transition-colors"
             >
               Close
             </button>
           </div>
                  ...
                  <div className={`bg-white/60 border border-white/50 shadow-macos-card rounded-xl p-5 space-y-4 transition-opacity ${!isLlmProvider ? 'opacity-50' : ''}`}>
                    ...
                    <div className="space-y-3">
                      <textarea
                        ...
                        className={`w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-macos-active focus:ring-2 focus:ring-macos-active/20 outline-none resize-none shadow-sm transition-all ${
                          !isLlmProvider || !systemPromptEnabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : ''
                        }`}
                      />
                      <div className="flex items-center justify-between">
                ...
                <>
                   ...
                   <div className="bg-white/60 border border-white/50 shadow-macos-card rounded-xl p-5 space-y-4">
                      <div className="flex items-center gap-2 text-sm font-medium">
                        ...
                        <span>Selection Translate Shortcut</span>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          <div
                            ...
                            className={`flex-1 px-4 py-3 rounded-lg border text-sm font-mono text-center cursor-pointer transition-all ${
                              isRecordingShortcut
                        ...
                        <p className="text-xs text-macos-muted">
                          {isRecordingShortcut
                            ...
                            : 'Click to record a new shortcut. This triggers the quick translation popup.'}
                        </p>
                      </div>
                   </div>
